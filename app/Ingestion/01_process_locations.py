...
final_prefix = "bronze/raw/locations/"

# 3. Write directly into final folder (Spark will create part files here)
locations_trans_df.write.options(header=True).format("csv").mode("overwrite").save(f"s3://{bucket_name}/{final_prefix}")

# 4. Rename part files in place with auto-increment pattern
s3 = boto3.client("s3")

# Find existing locations_NN.csv files to determine next suffix
objects = s3.list_objects_v2(Bucket=bucket_name, Prefix=final_prefix)
max_suffix = 0
pattern = re.compile(r"locations_(\d+)\.csv")
...
# Rename each part file to locations_XX_partN.csv
part_num = 1
for obj in objects.get("Contents", []):
    key = obj["Key"]
    if key.endswith(".csv") and "part" in key:  # Spark part files
        target_key = f"{final_prefix}locations_{next_suffix:02d}_part{part_num}.csv"
        s3.copy_object(
            Bucket=bucket_name,
...
print(f"Created {part_num-1} files with prefix locations_{next_suffix:02d}_partN.csv")
